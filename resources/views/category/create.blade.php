...
@section('content')
    <div class="container mx-auto">
        <h1 class="text-2xl font-bold mb-1.5">Add Category</h1>
        <p class="text-md text-gray-500 mb-8">Atur seluruh kategori kelas di platform.</p>

        <div class="grid grid-cols-1 w-2/3 gap-8">
            <div class="bg-white rounded-xl shadow-sm p-8">
                <h2 class="text-xl font-semibold mb-4">Category Icon</h2>
                
                <!-- Preview Icon (will display once file is chosen) -->
                ...
                <form action="{{ route('categories.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="name" name="name" class="w-full p-3 mt-2 border border-gray-300 rounded-full" required>
                        @error('name')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
