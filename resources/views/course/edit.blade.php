        ...
        <div class="grid grid-cols-1 w-2/3 gap-8">
            <div class="bg-white rounded-xl shadow-sm p-8">
                ...
                <div class="flex items-center gap-6 mb-4">
                    @if ($course->thumbnail)
                        <img id="previewIcon" src="{{ asset('storage/' . $course->thumbnail) }}" alt="{{ $course->name }}" class="w-80 h-40 rounded-xl border border-gray-300">
                    @else
                ...
                <form action="{{ route('courses.update', $course->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <button type="submit" class="w-1/3 py-3 bg-primary hover:opacity-90 cursor-pointer text-white rounded-full mt-4">Update Course</button>
