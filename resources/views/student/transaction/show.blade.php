...
@php
    ...
    $invoiceId = 'INV-' . $studentTrx->created_at->format('Ymd') . '-' . str_pad($studentTrx->id, 4, '0', STR_PAD_LEFT);
    $formattedDate = Carbon::parse($studentTrx->created_at)->format('d M Y, H:i');
@endphp

@section('student-content')
    <a href="{{ route('student.transaction') }}" class="inline-flex items-center text-font hover:text-primary mb-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-white rounded-lg p-6 shadow-md">
        <div class="flex flex-col">
            <img src="{{ asset('storage/' . $studentTrx->course->thumbnail) }}" class="w-80 h-auto rounded-md mb-4"
                alt="{{ $studentTrx->course->name }}">

            <h2 class="font-semibold text-xl mb-2">{{ $studentTrx->course->name }}</h2>
            <p class="text-gray-600 text-lg mb-4">Rp {{ number_format($studentTrx->amount) }}</p>

            <span
                class="text-sm px-4 py-1 rounded-full w-max
    {{ $studentTrx->status === 'completed'
        ? 'bg-green-100 text-green-700'
        : ($studentTrx->status === 'pending'
            ? 'bg-yellow-100 text-yellow-700'
            : 'bg-red-100 text-red-700') }}">
                {{ strtoupper($studentTrx->status) }}
            </span>
        </div>
        ...
        <div class="flex flex-col space-y-6">
            ...
            <div class="border-t pt-4">
                ...
                <div class="flex justify-between text-sm mb-2">
                    ...
                    <span>Rp {{ number_format($studentTrx->course_price) }}</span>
                </div>
                <div class="flex justify-between font-bold text-base">
                    <span>Total Transfer</span>
                    <span>Rp {{ number_format($studentTrx->amount) }}</span>
                </div>
            </div>
        </div>
    </div>
