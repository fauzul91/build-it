...
@php
    ...
    $invoiceId = 'INV-' . $adminTrx->created_at->format('Ymd') . '-' . str_pad($adminTrx->id, 4, '0', STR_PAD_LEFT);
    $formattedDate = Carbon::parse($adminTrx->created_at)->format('d M Y, H:i');
@endphp

@section('content')
    <a href="{{ route('admin.transaction') }}" class="inline-flex items-center text-font hover:text-primary mb-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-white rounded-lg p-6 shadow-md">
        <div class="flex flex-col">
            <img src="{{ asset('storage/' . $adminTrx->course->thumbnail) }}" class="w-80 h-auto rounded-md mb-4"
                alt="{{ $adminTrx->course->name }}">

            <h2 class="font-semibold text-xl mb-2">{{ $adminTrx->course->name }}</h2>
            <p class="text-gray-600 text-lg mb-4">Rp {{ number_format($adminTrx->amount) }}</p>

            <span
                class="text-sm px-4 py-1 rounded-full w-max
    {{ $adminTrx->status === 'completed'
        ? 'bg-green-100 text-green-700'
        : ($adminTrx->status === 'pending'
            ? 'bg-yellow-100 text-yellow-700'
            : 'bg-red-100 text-red-700') }}">
                {{ strtoupper($adminTrx->status) }}
            </span>
        </div>
        ...
        <div class="flex flex-col space-y-6">
            ...
            <div class="border-t pt-4">
                ...
                <div class="flex justify-between text-sm mb-2">
                    ...
                    <span>Rp {{ number_format($adminTrx->course_price) }}</span>
                </div>
                <div class="flex justify-between font-bold text-base">
                    <span>Total Transfer</span>
                    <span>Rp {{ number_format($adminTrx->amount) }}</span>
                </div>
            </div>
        </div>
    </div>
