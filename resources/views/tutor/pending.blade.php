...
@section('content')
    <div class="flex items-center justify-between">
        <div class="flex flex-col">
            <h1 class="text-2xl font-bold mb-1.5">Manage tutors</h1>
    ...
    <div class="flex flex-col items-center w-full bg-white rounded-2xl shadow-sm mt-8">
        @forelse ($tutors as $tutor)
            <div
                            ...
                            @if ($tutor->portofolio)
                                <div
                                    ...
                                    <img src="{{ asset('assets/images/icons/portfolio.svg') }}" alt="Portfolio Icon"
                                        class="w-5 h-5 flex-shrink-0">
                                    <div class="min-w-0">
                                        <p class="text-sm font-medium text-gray-700 truncate">Portfolio</p>
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto justify-end">
                    <form action="{{ route('tutor.approve', $tutor->id) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="flex items-center justify-center px-6 py-3 text-white bg-primary rounded-full hover:bg-opacity-90 cursor-pointer transition-colors text-sm font-medium whitespace-nowrap">
                            Approve
                        </button>
                    </form>
                    <form action="{{ route('tutor.reject', $tutor->id) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="flex items-center justify-center px-6 py-3 text-white bg-red-500 rounded-full hover:opacity-90 cursor-pointer transition-colors text-sm font-medium whitespace-nowrap">
                            Reject
                        </button>
                    </form>
                </div>
        ...
        @empty
            <p class="p-8">Belum ada tutor yang mengajukan.</p>
        @endforelse
    </div>
@endsection
